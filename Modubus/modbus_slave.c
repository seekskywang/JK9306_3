...
extern void uart1SendChars(uint8_t *str, uint16_t strlen);
extern void TIM5_Int_Init(uint16_t arr,uint16_t psc);
...
uint32_t  OldTick;	   //接收超时
unsigned int  Tick_10ms=0;    //时间片定时器
uint8_t uartpnt,uartlength;




struct MODS_T g_tModS;
struct VAR_Ta g_tVar = {
	80,
	1,
	2,
	3,
...
//	T0MCR = 0x03;           //匹配通道匹配中断复位     
//	T0MR0 = 110592/100;    //比较值1s/100=10ms;
////	T0TCR = 0x03;           //启动并复位定时器
//	T0TCR = 0x01;          
//    IRQ_Init(Time0_num,Time0_VectCnt,(uint32_t)IRQ_Timer0);		
//}
void MODS_Poll(void)
{
	uint16_t addr;
...
//void respon_Uart0_control(uint8_t l,uint8_t buf[])
//{
//     uartlength=l;
//     uartpnt=0;

//	   while(uartpnt<=uartlength)
//	   {
//	   	   U0THR = buf[uartpnt++];
...
//	RS485_SendBuf(buf, _ucLen);
	...
	uartSendChars(LPC_UART3,buf, _ucLen);
...
static uint8_t MODS_WriteRegValue(uint16_t reg_addr, uint16_t reg_value)
{
	...
	{	
		case SLAVE_REG_P00:
			g_tVar.P00 = reg_value;				/* 将值写入保存寄存器 */
			break;
		case SLAVE_REG_P01:
...
static void MODS_10H(void)
{
	/*
		...
		例子:
			...
			01 10 30 00 00 06 0C  07 DF  00 01  00 1F  00 17  00 3B  00 39     5549    ---- 写时钟 2016-01-31 23:59:57
