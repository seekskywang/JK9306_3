...
static __INLINE HCD_STATUS OHciRhPortPowerOn(uint8_t HostID, uint8_t uPortNumber)
{
	...
	{
		USB_REG(HostID)->RhPortStatus2 = HC_RH_PORT_STATUS_PortPowerStatus;	/* SetPortPower */
		HcdDelayMS(2 * ( (USB_REG(HostID)->RhDescriptorA & HC_RH_DESCRIPTORA_PowerOnToPowerGoodTime) >> 24 ) );	/* FIXME need to delay here POTPGT */
	}
...
static __INLINE HCD_STATUS OHciRhPortPowerOff(uint8_t HostID, uint8_t uPortNumber)
{
	...
	{
		USB_REG(HostID)->RhPortStatus2 = HC_RH_PORT_STATUS_LowSpeedDeviceAttached;	/* ClearPortPower */
	}
	return HCD_STATUS_OK;
}
...
static __INLINE HCD_STATUS OHciRhPortSuspend(uint8_t HostID, uint8_t uPortNumber)
{
	...
	{
		if ( USB_REG(HostID)->RhPortStatus2 & HC_RH_PORT_STATUS_CurrentConnectStatus) {	/* If device is connected */
			USB_REG(HostID)->RhPortStatus2 = HC_RH_PORT_STATUS_PortSuspendStatus;/* SetPortSuspend */
		}
	}
...
static __INLINE HCD_STATUS OHciRhPortResume(uint8_t HostID, uint8_t uPortNumber)
{
	...
	{
		if ( USB_REG(HostID)->RhPortStatus2 & HC_RH_PORT_STATUS_CurrentConnectStatus) {	/* If port is currently suspended */
			USB_REG(HostID)->RhPortStatus2 = HC_RH_PORT_STATUS_PortOverCurrentIndicator;	/* ClearSuspendStatus */
		}
	}
