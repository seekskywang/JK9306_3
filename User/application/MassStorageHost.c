 ...
 * this code.
 */
//#include "F:\lpc1788_dual_usb_host_msg\application\MassStorageHost.h"
...
static USB_ClassInfo_MS_Host_t FlashDisk2_MS_Interface = {
	.Config = {
		.DataINPipeNumber       = 1,
		.DataINPipeDoubleBank   = false,
...
static void USB_ReadWriteFile(void)
{
	...
	DEBUGOUT("\r\nTest completed.\r\n");
	USB_Host_SetDeviceConfiguration(FlashDisk1_MS_Interface.Config.PortNumber, 0);
}
...
void EVENT_USB_Host_DeviceEnumerationComplete(const uint8_t corenum)
{
	...
	FlashDisk1_MS_Interface.Config.PortNumber = corenum;
	if (MS_Host_ConfigurePipes(&FlashDisk1_MS_Interface,
							   ConfigDescriptorSize, ConfigDescriptorData) != MS_ENUMERROR_NoError) {
		DEBUGOUT("Attached Device Not a Valid Mass Storage Device.\r\n");
		return;
	}

	if (USB_Host_SetDeviceConfiguration(FlashDisk1_MS_Interface.Config.PortNumber, 1) != HOST_SENDCONTROL_Successful) {
		DEBUGOUT("Error Setting Device Configuration.\r\n");
		return;
	}

	uint8_t MaxLUNIndex;
	if (MS_Host_GetMaxLUN(&FlashDisk1_MS_Interface, &MaxLUNIndex)) {
	...
	if (MS_Host_ResetMSInterface(&FlashDisk1_MS_Interface)) {
	...
	if (MS_Host_RequestSense(&FlashDisk1_MS_Interface, 0, &SenseData) != 0) {
		DEBUGOUT("Error retrieving device sense.\r\n");
		USB_Host_SetDeviceConfiguration(FlashDisk1_MS_Interface.Config.PortNumber, 0);
		return;
	}

// 	if (MS_Host_PreventAllowMediumRemoval(&FlashDisk1_MS_Interface, 0, true)) {
...
// 	}
	...
	if (MS_Host_GetInquiryData(&FlashDisk1_MS_Interface, 0, &InquiryData)) {
...
{
	return &FlashDisk1_MS_Interface;
}
